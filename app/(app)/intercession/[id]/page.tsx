...
import { INTERCESSION_ENTRIES, getIntercessionCategory, getIntercessionEntryById, getIntercessionKind, testamentOf } from "@/lib/data/intercession";
...
export default function IntercessionDetailPage() {
  const params = useParams<{ id: string }>();
  const entry = getIntercessionEntryById(params.id);
  const [copied, setCopied] = useState(false);

  const bookmarked = useIntercessionStore((s) => (entry ? s.isBookmarked(entry.id) : false));
  const toggleBookmark = useIntercessionStore((s) => s.toggleBookmark);
  const openBibleAt = useFloatWindowsStore((s) => s.openBibleAt);

  const index = useMemo(() => (entry ? INTERCESSION_ENTRIES.findIndex((e) => e.id === entry.id) : -1), [entry]);
  const prev = useMemo(() => (index >= 0 ? INTERCESSION_ENTRIES[(index - 1 + INTERCESSION_ENTRIES.length) % INTERCESSION_ENTRIES.length] : null), [index]);
  const next = useMemo(() => (index >= 0 ? INTERCESSION_ENTRIES[(index + 1) % INTERCESSION_ENTRIES.length] : null), [index]);

  const related = useMemo(() => {
    if (!entry) return [];
    return INTERCESSION_ENTRIES.filter((e) => e.category === entry.category && e.id !== entry.id).slice(0, 3);
  }, [entry]);

  if (!entry) return notFound();

  const category = getIntercessionCategory(entry.category);
  const kind = getIntercessionKind(entry.kind);
    ...
    <div className="min-h-full" style={{ background: "var(--bj-bg)" }}>
      <div className="mx-auto px-5 md:px-12 pt-6 md:pt-10 pb-24" style={{ maxWidth: 760 }}>
        ...
        <motion.div variants={fadeUp} custom={1} initial="hidden" animate="show" className="flex items-center gap-2 mb-5 flex-wrap">
          ...
          >
            <IntercessionIcon name={category.icon} size={12} />
            {category.label}
          </span>
        ...
        {related.length > 0 && (
          <motion.section variants={fadeUp} custom={5} initial="hidden" animate="show">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link
                href={`/intercession?category=${category.id}`}
