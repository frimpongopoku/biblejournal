...
import { Handshake, Search, X, Bookmark, ArrowRight, Sparkles } from "lucide-react";
import { IntercessionIcon } from "@/components/intercession/IntercessionIcon";
import { IntercessionCard } from "@/components/intercession/IntercessionCard";
import { useIntercessionStore } from "@/store/intercession.store";
import {
  INTERCESSION_ENTRIES, INTERCESSION_CATEGORIES, INTERCESSION_KINDS, getIntercessionEntryById, intercessionEntryOfTheDay, testamentOf,
  type IntercessionCategoryId, type IntercessionKind,
} from "@/lib/data/intercession";
...
type KindFilter = "all" | IntercessionKind;

const CATEGORY_IDS = new Set(INTERCESSION_CATEGORIES.map((c) => c.id));

function IntercessionView() {
  const searchParams = useSearchParams();
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<IntercessionCategoryId | null>(() => {
    const c = searchParams.get("category");
    return c && CATEGORY_IDS.has(c as IntercessionCategoryId) ? (c as IntercessionCategoryId) : null;
  });
  const [testament, setTestament] = useState<TestamentFilter>("all");
  const [kind, setKind] = useState<KindFilter>(() => {
    const k = searchParams.get("kind");
    return k && INTERCESSION_KINDS.some((w) => w.id === k) ? (k as IntercessionKind) : "all";
  });

  const bookmarks = useIntercessionStore((s) => s.bookmarks);
  const bookmarkedEntries = useMemo(
    () => bookmarks.map((id) => getIntercessionEntryById(id)).filter((p): p is NonNullable<typeof p> => !!p),
    [bookmarks]
  );

  const daily = useMemo(() => intercessionEntryOfTheDay(), []);

  const q = query.trim().toLowerCase();
  const filtersActive = q.length > 0 || category !== null || testament !== "all" || kind !== "all";

  const filtered = useMemo(() => {
    return INTERCESSION_ENTRIES.filter((e) => {
      ...
      <div className="mx-auto px-5 md:px-12 pt-8 md:pt-12 pb-24" style={{ maxWidth: 1080 }}>
        ...
        <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show" className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            <Handshake size={16} style={{ color: "var(--bj-gold)" }} />
          ...
          <p className="font-sans text-sm max-w-xl" style={{ color: "var(--bj-ink4)", lineHeight: 1.65 }}>
            {INTERCESSION_ENTRIES.length} passages on the right we&apos;ve been given to pray for others — Christ&apos;s
            own intercession, the Spirit&apos;s, and the intercessors scripture holds up as examples. Promises,
            principles, warnings, and wisdom, each with the reference to study for yourself.
          </p>
        </motion.div>
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {INTERCESSION_CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  <IntercessionIcon name={c.icon} size={11} />
                  {c.label}
                </button>
              );
            })}
          ...
          <motion.section variants={fadeUp} custom={3} initial="hidden" animate="show" className="mb-9">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {bookmarkedEntries.map((e) => (
                <IntercessionCard key={e.id} entry={e} showCategory />
              ))}
            </div>
          </motion.section>
        )}
        ...
        {filtersActive ? (
          ...
          <div className="flex flex-col gap-10">
            {INTERCESSION_CATEGORIES.map((c, ci) => {
              const items = INTERCESSION_ENTRIES.filter((e) => e.category === c.id);
              return (
                <motion.section key={c.id} variants={fadeUp} custom={4 + ci * 0.4} initial="hidden" animate="show">
                  <div className="flex items-center gap-2.5 mb-4">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `color-mix(in oklch, ${c.color} 16%, var(--bj-bg))` }}>
                      <IntercessionIcon name={c.icon} size={12} style={{ color: c.color }} />
                    </div>
                    <h2 className="font-sans text-xs uppercase tracking-widest font-semibold" style={{ color: "var(--bj-ink4)" }}>
                      {c.label}
                    </h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {items.map((e) => (
                      <IntercessionCard key={e.id} entry={e} />
                    ))}
                  </div>
                </motion.section>
              );
