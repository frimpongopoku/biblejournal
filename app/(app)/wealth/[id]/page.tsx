...
export default function WealthDetailPage() {
  ...
  const entry = getWealthEntryById(params.id);
  ...
  const next = useMemo(() => (index >= 0 ? WEALTH_ENTRIES[(index + 1) % WEALTH_ENTRIES.length] : null), [index]);

  const related = useMemo(() => {
    if (!entry) return [];
    return WEALTH_ENTRIES.filter((e) => e.category === entry.category && e.id !== entry.id).slice(0, 3);
  }, [entry]);

  if (!entry) return notFound();

  const category = getWealthCategory(entry.category);
  const kind = getWealthKind(entry.kind);
    ...
    <div className="min-h-full" style={{ background: "var(--bj-bg)" }}>
      <div className="mx-auto px-5 md:px-12 pt-6 md:pt-10 pb-24" style={{ maxWidth: 760 }}>
        ...
        <motion.div variants={fadeUp} custom={1} initial="hidden" animate="show" className="flex items-center gap-2 mb-5 flex-wrap">
          ...
          >
            <WealthIcon name={category.icon} size={12} />
            {category.label}
          </span>
        ...
        {related.length > 0 && (
          <motion.section variants={fadeUp} custom={5} initial="hidden" animate="show">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link
                href={`/wealth?category=${category.id}`}
