...
import { Coins, Search, X, Bookmark, ArrowRight, Sparkles } from "lucide-react";
...
import {
  WEALTH_ENTRIES, WEALTH_CATEGORIES, WEALTH_KINDS, getWealthEntryById, wealthEntryOfTheDay, testamentOf,
  type WealthCategoryId, type WealthKind,
...
type KindFilter = "all" | WealthKind;

const CATEGORY_IDS = new Set(WEALTH_CATEGORIES.map((c) => c.id));

function WealthView() {
  const searchParams = useSearchParams();
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<WealthCategoryId | null>(() => {
    const c = searchParams.get("category");
    return c && CATEGORY_IDS.has(c as WealthCategoryId) ? (c as WealthCategoryId) : null;
  });
  const [testament, setTestament] = useState<TestamentFilter>("all");
  const [kind, setKind] = useState<KindFilter>(() => {
    const k = searchParams.get("kind");
    return k && WEALTH_KINDS.some((w) => w.id === k) ? (k as WealthKind) : "all";
  });

  const bookmarks = useWealthStore((s) => s.bookmarks);
  const bookmarkedEntries = useMemo(
    () => bookmarks.map((id) => getWealthEntryById(id)).filter((p): p is NonNullable<typeof p> => !!p),
    [bookmarks]
  );

  const daily = useMemo(() => wealthEntryOfTheDay(), []);

  const q = query.trim().toLowerCase();
  const filtersActive = q.length > 0 || category !== null || testament !== "all" || kind !== "all";

  const filtered = useMemo(() => {
    return WEALTH_ENTRIES.filter((e) => {
      ...
      <div className="mx-auto px-5 md:px-12 pt-8 md:pt-12 pb-24" style={{ maxWidth: 1080 }}>
        ...
        <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show" className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            <Coins size={16} style={{ color: "var(--bj-gold)" }} />
          ...
          <p className="font-sans text-sm max-w-xl" style={{ color: "var(--bj-ink4)", lineHeight: 1.65 }}>
            {WEALTH_ENTRIES.length} passages on provision, stewardship, and money — promises, principles, warnings,
            and wisdom side by side, each with the reference to study and the story behind it.
            Tap <span style={{ color: "var(--bj-gold-deep)", fontWeight: 500 }}>Promises</span> below to see only what God has pledged.
          </p>
        </motion.div>

        {/* ── Wealth entry of the day ───────────────────────── */}
        <motion.div variants={fadeUp} custom={1} initial="hidden" animate="show" className="mb-8">
          <Link
            href={`/wealth/${daily.id}`}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {WEALTH_CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  <WealthIcon name={c.icon} size={11} />
                  {c.label}
                </button>
              );
            })}
          ...
          <motion.section variants={fadeUp} custom={3} initial="hidden" animate="show" className="mb-9">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {bookmarkedEntries.map((e) => (
                <WealthCard key={e.id} entry={e} showCategory />
              ))}
            </div>
          </motion.section>
        )}
        ...
        {filtersActive ? (
          ...
          <div className="flex flex-col gap-10">
            {WEALTH_CATEGORIES.map((c, ci) => {
              const items = WEALTH_ENTRIES.filter((e) => e.category === c.id);
              return (
                <motion.section key={c.id} variants={fadeUp} custom={4 + ci * 0.4} initial="hidden" animate="show">
                  <div className="flex items-center gap-2.5 mb-4">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `color-mix(in oklch, ${c.color} 16%, var(--bj-bg))` }}>
                      <WealthIcon name={c.icon} size={12} style={{ color: c.color }} />
                    </div>
                    <h2 className="font-sans text-xs uppercase tracking-widest font-semibold" style={{ color: "var(--bj-ink4)" }}>
                      {c.label}
                    </h2>
