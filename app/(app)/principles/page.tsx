...
import { BookOpenCheck, Search, X, Bookmark, ArrowRight, Sparkles } from "lucide-react";
...
import {
  PRINCIPLES, PRINCIPLE_CATEGORIES, getPrincipleById, principleOfTheDay, testamentOf,
  type PrincipleCategoryId,
...
const CATEGORY_IDS = new Set(PRINCIPLE_CATEGORIES.map((c) => c.id));

function PrinciplesView() {
  const searchParams = useSearchParams();
  const [query, setQuery] = useState("");
  const [category, setCategory] = useState<PrincipleCategoryId | null>(() => {
    const c = searchParams.get("category");
    return c && CATEGORY_IDS.has(c as PrincipleCategoryId) ? (c as PrincipleCategoryId) : null;
  });
  const [testament, setTestament] = useState<TestamentFilter>("all");

  const bookmarks = usePrinciplesStore((s) => s.bookmarks);
  const bookmarkedPrinciples = useMemo(
    () => bookmarks.map((id) => getPrincipleById(id)).filter((p): p is NonNullable<typeof p> => !!p),
    [bookmarks]
  );

  const daily = useMemo(() => principleOfTheDay(), []);
      ...
      <div className="mx-auto px-5 md:px-12 pt-8 md:pt-12 pb-24" style={{ maxWidth: 1080 }}>
        ...
        <motion.div variants={fadeUp} custom={0} initial="hidden" animate="show" className="mb-8">
          <div className="flex items-center gap-2 mb-1">
            <BookOpenCheck size={16} style={{ color: "var(--bj-gold)" }} />
          ...
          <h1 className="font-display leading-none mb-3" style={{ fontSize: "clamp(1.8rem, 4vw, 2.4rem)", color: "var(--bj-ink)", fontWeight: 400, letterSpacing: "-0.01em" }}>
            Every law He has put in place.
          </h1>
          <p className="font-sans text-sm max-w-xl" style={{ color: "var(--bj-ink4)", lineHeight: 1.65 }}>
            {PRINCIPLES.length} governing truths woven through scripture — sowing and reaping, stewardship,
            justice, the power of words — each with the reference to study yourself and why it still holds.
          </p>
        </motion.div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <input
                ...
                placeholder="Search principles, references, or keywords…"
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {PRINCIPLE_CATEGORIES.map((c) => {
              ...
              return (
                ...
                >
                  <PrincipleIcon name={c.icon} size={11} />
                  {c.label}
                </button>
              );
            })}
        ...
        {/* ── Bookmarks ────────────────────────────────────── */}
        {bookmarkedPrinciples.length > 0 && !filtersActive && (
          <motion.section variants={fadeUp} custom={3} initial="hidden" animate="show" className="mb-9">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {bookmarkedPrinciples.map((p) => (
                <PrincipleCard key={p.id} principle={p} showCategory />
              ))}
            </div>
          </motion.section>
        )}
        ...
        {filtersActive ? (
          ...
          <div className="flex flex-col gap-10">
            {PRINCIPLE_CATEGORIES.map((c, ci) => {
              const items = PRINCIPLES.filter((p) => p.category === c.id);
              return (
                <motion.section key={c.id} variants={fadeUp} custom={4 + ci * 0.4} initial="hidden" animate="show">
                  <div className="flex items-center gap-2.5 mb-4">
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `color-mix(in oklch, ${c.color} 16%, var(--bj-bg))` }}>
                      <PrincipleIcon name={c.icon} size={12} style={{ color: c.color }} />
                    </div>
                    <h2 className="font-sans text-xs uppercase tracking-widest font-semibold" style={{ color: "var(--bj-ink4)" }}>
                      {c.label}
                    </h2>
