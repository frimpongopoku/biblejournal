...
import { getIntercessionCategory, getIntercessionKind, type IntercessionEntry } from "@/lib/data/intercession";

export function IntercessionCard({ entry, showCategory = false }: { entry: IntercessionEntry; showCategory?: boolean }) {
  const category = getIntercessionCategory(entry.category);
  const kind = getIntercessionKind(entry.kind);
  const bookmarked = useIntercessionStore((s) => s.isBookmarked(entry.id));
  const toggleBookmark = useIntercessionStore((s) => s.toggleBookmark);

  return (
    <Link
      href={`/intercession/${entry.id}`}
    ...
    >
      ...
      {showCategory && (
        ...
        >
          <IntercessionIcon name={category.icon} size={10} />
          {category.label}
        </span>
      )}
