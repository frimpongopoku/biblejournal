...
import { getWealthCategory, getWealthKind, type WealthEntry } from "@/lib/data/wealth";

export function WealthCard({ entry, showCategory = false }: { entry: WealthEntry; showCategory?: boolean }) {
  const category = getWealthCategory(entry.category);
  const kind = getWealthKind(entry.kind);
  const bookmarked = useWealthStore((s) => s.isBookmarked(entry.id));
  const toggleBookmark = useWealthStore((s) => s.toggleBookmark);

  return (
    <Link
      href={`/wealth/${entry.id}`}
    ...
    >
      ...
      {showCategory && (
        ...
        >
          <WealthIcon name={category.icon} size={10} />
          {category.label}
        </span>
      )}
